...
SubsPlayer 整合了视频字幕轨探测与批量提取、音频对齐、SRT 清洗、和多 Key 轮换AI翻译、断点续翻、硬字幕OCR翻译、双语 ASS 字幕生产的全部核心能力，并提供了一个基于 **PySide6 + mpv** 播放内核的流畅、直观、美观的桌面播放器模式。
...
> 主界面（编辑器在右布局）：左侧为 mpv 播放器实时渲染双语 ASS 字幕，右侧为可视化编辑器 —— 播放时自动高亮并滚动到当前字幕行，底部可对当前行进行时间戳数位微调、文本编辑与 OCR 识别。
...
*   **字幕轨探测与批量提取**
    *   通过 `ffprobe` 深度探测视频中内封的所有字幕轨，展示语言、编码格式、Default/Forced/SDH 属性。
    *   支持定义灵活的提取策略（例如 `eng`、`kor+sdh`、`auto`），一键应用于选中的所有视频，实现多线程批量提取。
*   **自动化时间轴对齐**
    *   集成 `alass`（Audio Language-Agnostic Subtitle Aligner）算法，支持直接以**视频原声轨**（自动通过 FFmpeg 提取音频）或**已知正确的参考字幕**作为基准，智能调整偏移并对齐错乱的 SRT 字幕时间轴。
*   **AI 智能翻译引擎**
    ...
    *   **自动术语表**：翻译前 AI 自动提取本片的人名、地名、组织名等专有名词并生成固定译法（保存为 `*.glossary.txt`，可手工编辑复用）；同一剧集的多个 SRT 会按文件名相似度自动复用同一套术语表，新出现的名词增量补充回共享表，保证整季译名统一；与设置中的自定义术语表自动合并（自定义条目优先）。
...
*   **国际化界面 (i18n)**
    *   全面支持 **中、英、日、韩、越、印尼、葡、西、法** 九国语言界面，重启立即生效。
...
### 一键启动
在项目根目录中直接双击 **`run.bat`**。
*   脚本会首先自动检查并安装 Python 依赖（`PySide6`、`python-mpv`、`requests`）。
*   检查无误后，自动启动 SubsPlayer 桌面应用程序。
...
2.  **字幕提取**（若有内封字幕）：在左侧面板的视频列表中选择视频，点击“提取字幕”即可看到该视频包含的音轨和字幕轨。批量配置应用后可一键将内封字幕提取为外部 SRT 格式。
3.  **时间对齐**：若字幕与视频音轨错位，点击“对齐字幕”，选择以“当前视频音轨”作为参考源，一键运行 `alass` 对齐。
4.  **AI 智能翻译**：选中字幕文件后，点击“AI 翻译”，配置您本次翻译的目标语言、是否导出双语、清洗规则等。翻译任务将多线程执行，如果您的 API Key 较多，可以在设置中填入多个，软件会自动轮换。
5.  **可视化微调**：翻译完毕后，字幕将自动加载进编辑器并实时在视频上渲染。你可以一边播放视频，一边在编辑器中校对译文和微调时间轴。
...
3.  **Gemini 翻译速度慢或总是报错？**
    *   国内用户需要确保网络环境能顺利访问 `generativelanguage.googleapis.com`。你可以在设置中增加 API Key 以提高轮换并发上限，或者配置代理（程序会自动读取系统全局代理）。
